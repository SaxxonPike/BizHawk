...
public sealed partial class Cia
{
	...
	*/
	...
	private byte _todSec;
	private byte _todMin;
	private byte _todHr;
	private byte _latch10Ths;
	private byte _latchSec;
	private byte _latchMin;
	private byte _latchHr;
	private byte _alm10Ths;
	private byte _almSec;
	private byte _almMin;
	private byte _almHr;
	private byte _sdr;
	private byte _icr;
	private byte _cra;
	...
	public Cia(int todNum, int todDen, Func<byte> readIec, Func<byte> readUserPort) : this(todNum, todDen)
	{
	...
	private void Ta_Idle()
	{
		if (_hasNewCra)
		{
			...
			_cra = unchecked((byte) (_newCra & 0xEF));
			_hasNewCra = false;
		}
	}
	...
	private void Tb_Idle()
	{
		if (_hasNewCrb)
		{
			...
			_crb = unchecked((byte) (_newCrb & 0xEF));
			_hasNewCrb = false;
		}
	}

	private void TriggerInterrupt(byte bit)
	{
		_icr |= bit;
